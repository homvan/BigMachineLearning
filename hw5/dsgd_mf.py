...
def map_line(line):
    ...
    word, doc, tfidf = line.strip().split(',')
    return (int(word), int(doc), float(tfidf))
...
def calculate_loss(pred_matrix, true_matrix):
    ...
    for worker_id, tuple in true_matrix:
        for word_id, doc_id, tf_idf in tuple:
            num_nonzero_entries += 1
            error += (pred_matrix[word_id][doc_id] - tf_idf) ** 2

    # TODO: calculate RMSE from error and num_nonzero_entries
    rmse = 0.5 * (error/num_nonzero_entries)
    print'error: %f, rmse: %f' % (error, rmse)
...
def blockify_matrix(worker_id, partition):
    ...
    for worker_id, tuple in partition:
        row_block_index = int(tuple[0]/strata_row_size)
        col_block_index = int(tuple[1]/strata_col_size)
        blocks[(row_block_index, col_block_index)].append(tuple)
...
def filter_block_for_iteration(num_workers, num_iteration, row_block_index, col_block_index):
    ...
    strata_index = col_block_index - row_block_index + num_workers

    return strata_index % num_workers == num_iteration % num_workers
...
def perform_sgd(block):
    ...
    row_start = row_block * strata_row_size # use col_block and row_block to transform to real row and col indexes
    col_start = col_block * strata_col_size # use col_block and row_block to transform to real row and col indexes
    ...
    for word_id, doc_id, tfidf_score in tuples:
        ...
        lrate = max(MIN_EPS, (100 + num_old_updates + num_updated) ** (-beta))
        # lrat = 10
        w_mat_block_part = np.copy(w_mat_block[word_id - row_start])
        h_mat_block_part = np.copy(h_mat_block[doc_id - col_start])
        loss = -2 * (tfidf_score - np.inner(w_mat_block_part, h_mat_block_part))

        # update W and H
        w_mat_block[word_id - row_start] = w_mat_block_part - lrate * loss * h_mat_block_part
        h_mat_block[doc_id - col_start] = h_mat_block_part - lrate * loss * w_mat_block_part
...
if __name__ == '__main__':
    ...
    max_word_id = tfidf_scores.map(lambda x: x[0]).reduce(lambda a, b: a if a > b else b)
    max_doc_id = tfidf_scores.map(lambda x: x[1]).reduce(lambda a, b: a if a > b else b)
    ...
    for current_iteration in range(num_iterations):
        ...
        updated = tfidf_scores \
            .filter(lambda s: filter_block_for_iteration(num_workers, current_iteration, s[0][0], s[0][1])) \
